...
if(isset($_SESSION['cus_fp_username'])){
    // include("inc/conn.php");
    include("inc/conn.php");

    $cus_fp_username = $_SESSION['cus_fp_username'];
    ...
    if ($result_login) {
        while ($row = mysqli_fetch_assoc($result_login)) {
            $users_login_img = $row['users_login_img'];
            $users_login_img = $users_login_img;
        }
    } else {
    ...
    <style>
        * {
        ...
        body {
            background-image: url("uploaded_img/<?php echo $users_login_img ?>");
    ...
    <a href="index.php"><button type="submit" class="btn btn-outline-primary mb-4 me-4">Back to home</button></a>
    ...
    $sql = "SELECT * FROM `cus_users` WHERE `cus_username` = '$cus_fp_username'";
    ...
    $email_message = "Hi " . $cus_fp_username . ", your otp for Lokeshwar fashion house is " . $db_otp . " ";
    ...
    include "inc/_company_info.php";
        ...
        $username = $_SESSION['cus_fp_username'];
    ...
    if ($mail->send()) {
        ...
        if (isset($_POST['submit_otp'])) {
            ...
            $sql = "SELECT * FROM `cus_users` WHERE `cus_username` = '$username'";
            ...
            if ($result) {
                ...
                if ($input_otp == $db_otp) {
                    ...
                    $cus_fp_username = $_SESSION['cus_fp_username'];
                    $sql_up = "UPDATE `cus_users` SET `cus_pass` = '$hash' WHERE `cus_users`.`cus_username` = '$cus_fp_username';";
