...
?>
    ...
    <div class="container full-width title_bar mt-4">
        ...
        $sql = "SELECT * FROM `newsletter` WHERE `id` = '$get_id'";
        ...
        if ($result) {
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $newsletter_ip =   $row['ip_address'];
                    $newsletter_os =   $row['os'];
                    $newsletter_browser =   $row['browser'];
                    $newsletter_device =   $row['device'];
                    $newsletter_subscribed_datetime =  $row['datetime'];
...
<a href = "newsletter.php" class="mb-4"><button class = "btn btn-primary mb-4">Go to newsletter</button></a><br>
    ...
    Newsletter IP Address (in the time of subscribe): ' . $newsletter_ip . ' <br>
    Newsletter OS (in the time of subscribe): ' . $newsletter_os . ' <br>
    Newsletter Browser (in the time of subscribe): ' . $newsletter_browser . ' <br>
    Newsletter Device (in the time of subscribe): ' . $newsletter_device . ' <br>

    Newsletter subscribed Email: ' . $newsletter_email . ' <br>
    
    Newsletter subscribed datetime: ' . $newsletter_subscribed_datetime . ' <br>
...
// }
                ...
                echo '<div class = "custom-default-box-bg-color pt-4 mt-4 ps-4 fs-4" style="height:200px !important;">
      Sorry ! the searching newsletter details are not found ..
