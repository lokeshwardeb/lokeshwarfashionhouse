...
function mail_template($website_name = "Lokeshwar Fashion House", $select_template, $username, $order_no = '', $order_phone_no = ''){
    include "_company_info.php";
...
// $username = '';
  ...
  ");
    ...
    if($select_template == 'verify_your_email'){
        ...
        $sql_get_otp = "SELECT * FROM `cus_users` WHERE `cus_username` = '$username'";


        $result_get_otp = mysqli_query($conn, $sql_get_otp);
        ...
        if($result_get_otp){
            ...
            $update_otp_sql = "UPDATE `cus_users` SET `otp`='$otp' WHERE `cus_username`= '$username'";
            $update_otp_result = mysqli_query($conn, $update_otp_sql);


            // if(mysqli_num_rows($result_get_otp) > 0){
            //     while ($row = mysqli_fetch_assoc($result_get_otp)) {
            //         $otp = $row['otp'];
            //     }
            // }
    ...
    if($select_template == 'order_placed'){
        include "conn.php";
        include "_company_info.php";
        ...
        $order_no_new = htmlspecialchars($order_no, ENT_QUOTES);

        $order_no;

       $order_no_mail_link =  trim($order_no, "#");
        ...
        Your order $order_no has been placed successfully and we will let you know once your package is on its way. Check the status of your order using the tracking link below . And you can also check the order status from our platform.<br> <br>

        Order no : $order_no_new;
        <br> <br>

        <center>
        <button style='background-color: #0d6efd;'><a style='text-decoration: none; color:white' href='".SITE_URL."order_tracking.php?search_text_order_track_no=%23$order_no_mail_link&search_text_order_track_pho_no=$order_phone_no&track_order=orders'>Track your order</a></button>
        </center>
        ...
        Need help ? How can I track my order ? <br><br>

        Log in to your user account and go my orders to see all of your orders. <br><br>

        Login to user account -> My orders -> your all orders <br><br>

        You can also track your orders from track my orders option <br><br>


        Thanks for purchasing from us . -- ".$website_name."<br>
